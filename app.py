...
def find_data_file(data_dir: str = 'data') -> str | None:
    candidates = [
        'ARRECADAO.xlsx', 'ARRECADAÇÃO.xlsx', 'ARRECADACAO.xlsx',
    ...
    if os.path.isdir(data_dir):
        for f in os.listdir(data_dir):
            # Ignora arquivos de lock temporários gerados pelo Excel
            if f.startswith('~$'):
                continue
...
if file_path is None:
    st.error(
        ...
        "Por favor coloque 'ARRECADAO.xlsx' ou 'ARRECADAÇÃO.xlsx' em data/."
    )
...
with col3:
    ...
    label_red = (
        f"font-size:13px; color:{PRIMARY_COLOR}; margin:0; "
        f"font-weight:700; text-transform:uppercase;"
    )
    label_normal = (
        f"font-size:13px; color:{NAVY}; margin:0; "
        f"font-weight:700; text-transform:uppercase;"
    )
